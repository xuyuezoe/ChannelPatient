...
## 追加（2026-09-15 晚）：目标函数改为风险加权（RVOI）后的重跑

决定：只做一篇论文，风险加权目标直接进论文一。损失矩阵：漏红旗 10、误报红旗 2、良性间错判 1、移交 1.5、每问一句 0.05。停止规则 = 没有候选能把期望损失降得超过一句的成本。

| channel       | doctor            |   kl_mean |   kl_final |   clarification_rate |   verify_usage |   readout_parse_rate |   accuracy_final |   mirage_gap |   turns_mean |
|:--------------|:------------------|----------:|-----------:|---------------------:|---------------:|---------------------:|-----------------:|-------------:|-------------:|
| cooperative   | agent             |     0.217 |      0     |                0.465 |              0 |                    1 |            1     |        0.313 |        4.909 |
| cooperative   | agent_nolookahead |     0.225 |      0     |                0.242 |              0 |                    1 |            1     |        0.343 |        4     |
| cooperative   | agent_zonly       |     0.203 |      0.003 |                0.383 |              0 |                    1 |            1     |        0.411 |        5.273 |
| cooperative   | scripted_fixed    |   nan     |    nan     |                0.091 |              1 |                    0 |            0.273 |        0     |       13     |
| exaggerate_k2 | agent             |     0.217 |      0     |                0.465 |              0 |                    1 |            1     |        0.313 |        4.909 |
| exaggerate_k2 | agent_nolookahead |     0.225 |      0     |                0.242 |              0 |                    1 |            1     |        0.343 |        4     |
| exaggerate_k2 | agent_zonly       |     0.231 |      0.004 |                0.365 |              0 |                    1 |            1     |        0.427 |        5.364 |
| exaggerate_k2 | scripted_fixed    |   nan     |    nan     |                0.091 |              1 |                    0 |            0.273 |        0     |       13     |
| self_dx       | agent             |     0.217 |      0     |                0.465 |              0 |                    1 |            1     |        0.313 |        4.909 |
| self_dx       | agent_nolookahead |     0.225 |      0     |                0.242 |              0 |                    1 |            1     |        0.343 |        4     |
| self_dx       | agent_zonly       |     0.203 |      0.003 |                0.383 |              0 |                    1 |            1     |        0.411 |        5.273 |
| self_dx       | scripted_fixed    |   nan     |    nan     |                0.091 |              1 |                    0 |            0.273 |        0     |       13     |
| vague_p08     | agent             |     0.217 |      0     |                0.465 |              0 |                    1 |            1     |        0.313 |        4.909 |
| vague_p08     | agent_nolookahead |     0.225 |      0     |                0.242 |              0 |                    1 |            1     |        0.343 |        4     |
| vague_p08     | agent_zonly       |     0.2   |      0.002 |                0.389 |              0 |                    1 |            1     |        0.395 |        5.318 |
| vague_p08     | scripted_fixed    |   nan     |    nan     |                0.091 |              1 |                    0 |            0.273 |        0     |       13     |


### 变化
- 完整 agent 现在**按需问红旗排除类问题**：22 段里劳力诱发问了 16 次、放射 10 次、前倾缓解 10 次（之前一次都不问）。不是清单：心绞痛质量被压低后就不再问。澄清率从 0.33 升到 0.47（强制选项的价值在风险目标下更高）。
- 平均轮数 5.7 → 4.9，准确率仍 1.0，终局 KL 仍 0：多问的是排除红旗的问题，少问的是对风险无关的问题。
- Z-only 的差距在 stub 环境里仍然小（终局 KL 0.003-0.004 vs 0），原因不变：最优路径不经过失真槽。四种信道下完整 agent 仍完全相同——**夸大和模糊要显现，得靠被测医生和固定问卷的对照，或第二批信道把夸大扩到性质和诱因**。
- 去前瞻的 agent 轮数最短（4.0）、澄清率最低（0.24）：在风险目标下前瞻的作用比信息增益目标下明显。

### 风格旋钮
`style: concise | detailed | warm` 已接入（模板版零调用，LLM 版每轮 1 次并复核槽位不变）。D-M3 全部用 concise；风格只改措辞，不改本表任何数字。
